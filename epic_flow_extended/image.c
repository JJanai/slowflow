...
image_t *image_new(const int width, const int height){
    ...
    image->data = (float*) memalign(16, image->stride*height*sizeof(float));		// uninitialized array
    if(image->data == NULL){
        fprintf(stderr, "Error: image_new() - not enough memory !\n");
        exit(1);
    }
//	memset(image->data, 0, image->stride*image->height*sizeof(float));				// zero initialization (replaced by image_erase if necessary)
    return image;
}
...
color_image_t *color_image_new(const int width, const int height){
    ...
    image->height = height;  
    image->stride = ( (width+3) / 4 ) * 4;
    image->c1 = (float*) memalign(16, 3*image->stride*height*sizeof(float));		// uninitialized array
    if(image->c1 == NULL){
        fprintf(stderr, "Error: color_image_new() - not enough memory !\n");
        exit(1);
    }
//	memset(image->c1, 0, 3*image->stride*height*sizeof(float));						// zero initialization (replaced by color_image_erase if necessary)
...
void image_rescale(image_t const *src, image_t *dst, float factor, int method) {
	int i, j, offset, x, y, x1, x2, y1, y2;
	float xx, yy, dx, dy;
	for(j=0 ; j < dst->height ; j++){
		offset = j*dst->stride;
		for(i=0 ; i < dst->width ; i++,offset++){
			xx = i / factor;
			yy = j / factor;
			...
			if(method == INTERP_LINEAR) {
				...
				x = RECTIFY(x, src->width);
				y = RECTIFY(y, src->height);

				dst->data[offset] = src->data[y*src->stride+x];
			}
		}
	}
}

void image_rescale_xy(image_t const *src, image_t *dst, float fx, float fy, int method) {
	int i, j, offset, x, y, x1, x2, y1, y2;
	float xx, yy, dx, dy;
	for(j=0 ; j < dst->height ; j++){
		offset = j*dst->stride;
...
void color_image_rescale(color_image_t const *src, color_image_t *dst, float factor) {
	int i, j, offset, x, y, x1, x2, y1, y2;
	float xx, yy, dx, dy;
	for(j=0 ; j < dst->height ; j++){
		offset = j*dst->stride;
		...
		for (i = 0; i < dst->width; i++, offset++) {
			...
			dst->c1[offset] = src->c1[y1 * src->stride + x1] * (1.0f - dx) * (1.0f - dy)
					+ src->c1[y1 * src->stride + x2] * dx * (1.0f - dy)
					+ src->c1[y2 * src->stride + x1] * (1.0f - dx) * dy
					+ src->c1[y2 * src->stride + x2] * dx * dy;
			dst->c2[offset] = src->c2[y1 * src->stride + x1] * (1.0f - dx) * (1.0f - dy)
					+ src->c2[y1 * src->stride + x2] * dx * (1.0f - dy)
					+ src->c2[y2 * src->stride + x1] * (1.0f - dx) * dy
					+ src->c2[y2 * src->stride + x2] * dx * dy;
			dst->c3[offset] = src->c3[y1 * src->stride + x1] * (1.0f - dx) * (1.0f - dy)
					+ src->c3[y1 * src->stride + x2] * dx * (1.0f - dy)
					+ src->c3[y2 * src->stride + x1] * (1.0f - dx) * dy
					+ src->c3[y2 * src->stride + x2] * dx * dy;

//			printf("%f,%f,%f\n", dst->c1[offset], dst->c2[offset],  dst->c3[offset]);
//			printf("%f,%f,%f\n", src->c1[y1 * src->stride + x1], src->c2[y1 * src->stride + x1],  src->c3[y1 * src->stride + x1]);
//			printf("%f,%f,%f\n", factor, xx, yy);
		}
	}
}

void color_image_rescale_xy(color_image_t const *src, color_image_t *dst, float fx, float fy) {
	int i, j, offset, x, y, x1, x2, y1, y2;
	float xx, yy, dx, dy;
	for(j=0 ; j < dst->height ; j++){
		offset = j*dst->stride;
...
void color_image_rescale_NN(color_image_t const *src, color_image_t *dst, float factor) {
	int i, j, offset, x, y;
	float xx, yy, dx, dy;
	for(j=0 ; j < dst->height ; j++){
		offset = j*dst->stride;
		...
		for (i = 0; i < dst->width; i++, offset++) {
			...
			dst->c1[offset] = src->c1[y * src->stride + x];
			dst->c2[offset] = src->c2[y * src->stride + x];
			dst->c3[offset] = src->c3[y * src->stride + x];
